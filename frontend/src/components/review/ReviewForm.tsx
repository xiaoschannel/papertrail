    ...
    <div className="review-form">
      ...
      <div className="field">
        <label htmlFor="rv-smart">
          ...
          {doc.smart_matches.length ? ` — best ${Math.round(bestNameScore * 100)}%` : ''}
        </label>
        <select id="rv-smart" value="" onChange={(e) => e.target.value && onChange({ name: e.target.value })}
          ...
          {doc.smart_matches.map((m) => <option key={m.name} value={m.name}>{m.name}</option>)}
        </select>
      </div>
          ...
          <div className="field">
            ...
            <div className="currency">
              ...
              <button className={form.currency === 'JPY' ? 'on' : ''} aria-pressed={form.currency === 'JPY'}
                title="Set currency to JPY" onClick={() => onChange({ currency: 'JPY' })}>JPY</button>
            </div>
          </div>
        </div>
      )}
