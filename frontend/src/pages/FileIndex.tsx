    ...
    <Card title="Batches" hint={`${s.existing_batches} batch(es) indexed`}>
      ...
      {s.error && (
        <div className="error-banner" role="alert">
          {s.error}
          {s.offending.length > 0 && <ul className="file-list">{s.offending.map((f) => <li key={f}>{f}</li>)}</ul>}
        </div>
      )}
      ...
      {!s.error && s.proposal.length === 0 && <p className="ingest-note">No new files to index.</p>}
    </Card>
  )
}
...
function GroupingEditor({ data, batchId, onBatch }: { data: Grouping; batchId: number; onBatch: (id: number) => void }) {
  ...
  const pager = pageCount > 1 && (
    <div className="pager">
      ...
      <span>Page {shownPage + 1} of {pageCount}</span>
  ...
  return (
    <Card title="Document grouping" hint="Link adjacent pages into one document; ⇄ swaps the order of linked pages.">
      ...
      <div className="start-bar">
        ...
        {!changed && <span className="ingest-note">No unsaved changes.</span>}
      </div>
...
function PageTile({ page, group, link, busy, rotateLocked, tossLocked, onRotate, onToss }: {
  page: GroupingPage
