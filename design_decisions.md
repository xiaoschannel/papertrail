...
The repo has multi-provider setup for flexibility. In my testing, Deepseek OCR 2 and a hosted model for batch processing, and for receipts that Deepseek OCR 2 failed, GLM-OCR can often get it right. Qwen3-8b is usable if you want to process everything fully locally, but a hosted model's better accuracy really cuts down the manual re-editing. That testing was done with gpt5.4; the hosted extractors are now GPT-5.6 Luna, cheap enough to run a whole batch through, and Terra for the awkward ones.
